...
namespace CollectCER
{
    class Program
    {
         ...
         * status is -26 in the last 7 days
         */
        ...
        private static void DBProcess()
        {
            ...
            try
            {
                ...
                Console.WriteLine("There is no -26 CERs in the last 7 days.");
            }
            catch (Exception e)
            {
        ...
        private static int QueryCERNumber()
        {
            ...
            try
            {
                conn.Open();
                OracleCommand cmd = conn.CreateCommand();
                string queryCode = "SELECT COUNT(*) FROM CER_ERROR_REPORT t WHERE t.status = -26 and (sysdate - daterecieved) <7";
                cmd.CommandText = queryCode;
                ...
                string insertCode = "INSERT INTO CER_REPROCESSING_REQUESTS " +
                                    ...
                                    "VALUES " +
                                        "(" +
                                            ...
                                            //Comment this for testing.
                                            //"'" + "select ... " + "', " +
                                            "'" + "select id from cer_error_report where status=-26 and (sysdate - daterecieved) <7" + "', " +
                                            "1" +
                                        ")";
